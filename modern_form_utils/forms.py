...
class Fieldset:
    def __init__(self, form, name, boundfields, legend='', classes='', description=''):
        ...
        self.legend = mark_safe(legend or name or '')
...
class FieldsetCollection:
    ...
    def _gather_fieldsets(self):
        if not self.fieldsets:
            self.fieldsets = [('main', {'fields': list(self.form.fields.keys()), 'legend': ''})]
        for name, options in self.fieldsets:
            ...
            self._cached_fieldsets.append(Fieldset(
...
def _get_meta_attr(meta_class, attr, default):
    return getattr(meta_class, attr, default)
...
def get_fields_from_fieldsets(fieldsets):
    ...
    for name, options in fieldsets:
        try:
            ...
            raise ValueError(
                '"fieldsets" must be an iterable of two-tuples, and each second element must '
                'be a dictionary with a "fields" key'
            )
...
class BetterFormBaseMetaclass(type):
    def __new__(cls, name, bases, attrs):
        ...
        if not hasattr(meta, 'fields') and not hasattr(meta, 'exclude'):
            setattr(meta, 'fields', get_fields_from_fieldsets(fieldsets))
...
class BetterForm(forms.Form, metaclass=type('BetterFormMetaclass', (BetterFormBaseMetaclass, forms.forms.DeclarativeFieldsMetaclass), {})):
...
class BetterModelForm(forms.ModelForm, metaclass=type('BetterModelFormMetaclass', (BetterFormBaseMetaclass, forms.models.ModelFormMetaclass), {})):
    def __init__(self, *args, **kwargs):
        self._fieldsets = deepcopy(self.base_fieldsets)
        self._row_attrs = deepcopy(self.base_row_attrs)
        self._fieldset_collection = None
